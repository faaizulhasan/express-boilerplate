...
const { CHAT_ROOM_TYPE_ENUM, BADGE_TYPE_ENUM, MESSAGE_TYPE_ENUM } = require("../../../config/enum");
...
class ChatRoomController extends SocketRestController {
    ...
    async createGroup({ io, socket }) {
        try {
            ...
            const rules = {
                ...
                "members": "required|array_string|min:1",
            }
            ...
            for (let i = 0; i < params.members; i++) {
                ...
                if (user_slug === this.socket.user.slug) {
                    return this.sendError(
                        "Not able to create group with you",
                        {},
                        400
                    )
                }

                const user = await User.instance().validateUser(user_slug)
                if (_.isEmpty(user)) {
                    return this.sendError(
                        "Invalid player id",
                        {},
            ...
            this.socket.join('room_' + record.slug)
            ...
            this.resource = 'Group'
            const group_record = await this.modal.getRoomRecordWithUserDetails(record.slug)
            this.emitter.push((obj) => this.socket.emit('createGroup_', obj))
            console.log("Member added emitter to user :  ", params.members);
            params.members.forEach((user_slug) => {
                this.emitter.push((obj) => this.io.in('user_' + user_slug).emit('newGroup_', obj))
            })
    ...
    async updateGroupDetails({ io, socket }) {
        try {
            ...
            const chat_room_record = await this.modal.getRoomRecordWithUser(user.slug, params.chat_room_slug);
            if (_.isEmpty(chat_room_record) || (chat_room_record.type !== CHAT_ROOM_TYPE_ENUM.GROUP)) {
                return this.sendError(
            ...
            if (!chat_room_record.can_memberEditGroup && (chat_room_record.user_slug !== user.slug)) {
                return this.sendError(
                    "Only admin are able to edit group",
                    {},
                    400
                )
            }
            ...
            const record = await this.modal.updateRecord(this.socket, payload, params.chat_room_slug);
            ...
            this.resource = 'CreateGroup'
            this.emitter.push((obj) => this.io.in('room_' + params.chat_room_slug).emit('updateGroupDetails_', obj))
            await this.sendResponse(
                200,
                "Group details updated successfully",
                record
